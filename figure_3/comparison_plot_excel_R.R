...
excel_model_results <- read.csv("excel_model_results.csv")
...
for (file in files) {
  ...
  excel_model_result <- excel_model_results[excel_model_results$file_name == file_name, ]

  row <- list(
    "file_name" = file_name,
    # eilers peeters
    "eilers_peeters_etr_r" = as.numeric(r_eilers_peeters_model_result$etrmax_with_photoinhibition),
    "eilers_peeters_etr_excel" = as.numeric(excel_model_result$etrmax_2_ep),
    "eilers_peeters_ik_r" = as.numeric(r_eilers_peeters_model_result$ik_with_photoinhibition),
    "eilers_peeters_ik_excel" = as.numeric(excel_model_result$ik_2_ep),
    "eilers_peeters_alpha_r" = as.numeric(r_eilers_peeters_model_result$alpha),
    "eilers_peeters_alpha_excel" = as.numeric(excel_model_result$alpha_2_ep),
    # platt
    "platt_etr_r" = as.numeric(r_platt_model_result$etrmax_with_photoinhibition),
    "platt_etr_excel" = as.numeric(excel_model_result$etrmax_2_platt),
    "platt_ik_r" = as.numeric(r_platt_model_result$ik_with_photoinhibition),
    "platt_ik_excel" = as.numeric(excel_model_result$ik_2_platt),
    "platt_alpha_r" = as.numeric(r_platt_model_result$alpha),
    "platt_alpha_excel" = as.numeric(excel_model_result$alpha_2_platt)
  )
...
make_regression_plot <- function(df, shape, nudge_x = 0, nudge_y = 0) {
  ...
  # outlier calculation
  df[, rel_diff := abs((value_R - value_excel) / value_R) * 100]
  df[, outlier := rel_diff > 3]

  # regression fit
  lm_clean <- lm(value_R ~ value_excel, data = df[outlier == FALSE])
  ...
  # axis scaling
  max <- max(c(df$value_excel, df$value_R), na.rm = TRUE)
  min <- min(c(df$value_excel, df$value_R), na.rm = TRUE)
  ...
  # plot generation
  ggplot(df, aes(x = value_excel, y = value_R)) +
    theme_base() +
    theme(
    ...
    geom_smooth(data = df[outlier == FALSE], aes(x = value_excel, y = value_R), method = "lm", se = FALSE, color = "black") +
...
plot_etrmax_platt <- make_regression_plot(
  data.table(
    ...
    value_excel = result$platt_etr_excel
  ),
  shape = 16,
  nudge_x = 16
) + legend_platt + labs(
  ...
  x = bquote(ETR[max] ~ "(" * mu * "mol electrons" ~ m^-2 ~ s^-1 * ") (E)")
)
...
plot_ik_platt <- make_regression_plot(
  data.table(
    ...
    value_excel = result$platt_ik_excel
  ),
  shape = 16,
  nudge_x = 18
) + legend_platt + labs(
  ...
  x = bquote(I[k] ~ "(" * mu * "mol photons" ~ m^-2 ~ s^-1 * ") (E)")
)
...
plot_alpha_platt <- make_regression_plot(
  data.table(
    ...
    value_excel = result$platt_alpha_excel
  ),
  shape = 16,
  nudge_y = 20
) + legend_platt + labs(
  ...
  x = bquote(alpha ~ "(electrons / photons) (E)")
)
...
plot_etrmax_eilers_peeters <- make_regression_plot(
  data.table(
    ...
    value_excel = result$eilers_peeters_etr_excel
  ),
  shape = 15,
  nudge_x = 16
...
plot_ik_eilers_peeters <- make_regression_plot(
  data.table(
    ...
    value_excel = result$eilers_peeters_ik_excel
  ),
  shape = 15,
  nudge_y = 20
) + legend_eilers_peeters + labs(
...
plot_alpha_eilers_peeters <- make_regression_plot(
  data.table(
    ...
    value_excel = result$eilers_peeters_alpha_excel
  ),
  shape = 15,
  nudge_x = 20
) + legend_eilers_peeters + labs(
